 ...
 * @since 1.0.0
 */
export const to = schema => go(schema.ast);
/**
 * @category arbitrary
 * @since 1.0.0
 */
export const from = schema => go(AST.getFrom(schema.ast));
...
const go = ast => {
  switch (ast._tag) {
    case "Declaration":
      return pipe(getHook(ast), O.match(() => go(ast.type), handler => handler(...ast.typeParameters.map(go))));
    case "Literal":
    ...
    case "Transform":
      {
        const to = go(ast.to);
        return pipe(getHook(ast), O.match(() => fc => to(fc).filter(a => {
