 ...
 * @since 1.0.0
 */
export const from = schema => compile(AST.getFrom(schema.ast));
 ...
 */
export const match = {
  "Declaration": (ast, go) => pipe(getHook(ast), O.match(() => go(ast.type), handler => handler(...ast.typeParameters.map(go)))),
  "VoidKeyword": () => () => "void(0)",
  "NeverKeyword": () => () => {
  ...
  "Union": (ast, go) => {
    const types = ast.types.map(ast => [P.is(I.makeSchema(ast)), go(ast)]);
    return a => {
